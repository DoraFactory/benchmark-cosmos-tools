...
async function batchSend(recipients: string[]) {
...
async function benchmarkTest(thread: number, packageSize: number) {
  // let thread = 10000;
...
async function main() {
  let pool = 10;
  let thread = 1000;
  let packageSize = 1000;

  let totalTxs = pool * thread * packageSize;
  console.log(`Total Txs: ${totalTxs}`);
  for (let i = 0; i < pool; i++) {
    await benchmarkTest(thread, packageSize);
  }
}
